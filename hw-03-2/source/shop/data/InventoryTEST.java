...
public class InventoryTEST extends TestCase {
  ...
  public void testAddNumOwned() {
          ...
          Record oldCopy = s.addNumOwned(v1, 1);     Assert.assertEquals( v1, s.get(v1).video() );
                                    Assert.assertEquals( 1, s.get(v1).numOwned());
                                    Assert.assertEquals(oldCopy, null);
  ...
  public void testCheckOutCheckIn() {
    ...
    try { s.checkIn(null);      Assert.fail(); } catch ( IllegalArgumentException e ) {}
          ...
          Record compareCopy = s.get(v1); Record oldCopy = s.checkOut(v1);     
          Assert.assertTrue( s.get(v1).numOut() == 1 && s.get(v1).numRentals() == 1 );
          Assert.assertTrue(oldCopy.equals(compareCopy));  s.addNumOwned(v1,-1); 
          Assert.assertTrue( s.get(v1).numOut() == 1 && s.get(v1).numRentals() == 1 );
    ...
    try { s.checkOut(v1);       Assert.fail(); } catch ( IllegalArgumentException e ) {}
          Record compareCopy2 = s.get(v1); Record oldCopy2 = s.checkIn(v1);  
          Assert.assertTrue(compareCopy2.equals(oldCopy2));
  ...
  public void testClear() {
	  	  Map<Video, Record> oldS = new HashMap<>(); 
	  	  Map copy = s.clear(); Assert.assertEquals(oldS, copy);
          s.addNumOwned(v1, 2); Assert.assertEquals( 1, s.size() );
