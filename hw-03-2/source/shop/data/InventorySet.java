...
final class InventorySet implements Inventory {
  ...
  public Iterator<Record> iterator(Comparator<Record> comparator) {
	  List<Record> r = new ArrayList<Record>();
		 for (Record recs: _data.values()) {
			 r.add(recs);
		 }
	    Collections.sort(r, comparator);
	    
	    return Collections.unmodifiableCollection(r).iterator();
  }
  ...
  Record checkOut(Video video) {
	  RecordObj r = (RecordObj)_data.get(video);
	  if (!(_data.containsKey(video)) || (r.numOut == r.numOwned)){
	    	throw new IllegalArgumentException("unable to check out video");
	    }
	    else {
  ...
  Record checkIn(Video video) {
	  RecordObj r = (RecordObj)_data.get(video);
	  if (!(_data.containsKey(video)) || r.numOut <= 0){
	    	throw new IllegalArgumentException("unable to check in video");
	    }
	  else {
   ...
   */
  Map clear() {
   Map<Video, Record> d = new HashMap<>();
   d.putAll(_data);
   _data.clear();
   return d;
  }
